...
{
	public ConsolaAdminLocal(AlquilerVehiculos alquilerVehiculos,
			Usuario usuario)
	{
		super(alquilerVehiculos);
	...
	private static void agregarAuto()
	{
		try (Scanner scanner = new Scanner(System.in))
		{
			...
			// Crear un objeto de la clase Vehiculo con los datos ingresados por
			// el usuario
			Vehiculo nuevoVehiculo = new Vehiculo(placa, categoria, marca,
					modelo, color, transmision, ubicacion);
	...
	/*
	 * public void quitarAuto(String placa) { if (autenticado) { // Lógica para
	 * quitar un auto boolean eliminado =
	 * AlquilerVehiculos.quitarVehiculo(placa); if (eliminado) {
	 * System.out.println("Vehículo con placa " + placa +
	 * " eliminado del sistema."); } else {
	...
	private static void modificarAuto()
	{
		try (Scanner scanner = new Scanner(System.in))
		{
			...
			// Crear objetos de la clase Vehiculo para el vehículo antiguo y el
			// nuevo
			Vehiculo vehiculoViejo = new Vehiculo(placaViejo, categoria, marca,
					modelo, color, transmision, ubicacion);
			Vehiculo vehiculoNuevo = new Vehiculo(placaNuevo, categoria, marca,
					modelo, color, transmision, ubicacion);

			// Llamar al método modificarVehiculo y pasar los objetos de
			// vehículo antiguo y nuevo como argumentos
			boolean modificado = alquilerVehiculos
					.modificarVehiculo(vehiculoViejo, vehiculoNuevo);
			...
			{
				System.out.println(
						"No se pudo modificar el vehículo. Verifique los datos ingresados.");
			}
		}
	}
	...
	public void mostrarMenu()
	{
		...
		do
		{
			...
			switch (opcion)
			{
				...
				case 6 :
					System.out.println(
							"Has seleccionado administrar sedes y empleados.");
